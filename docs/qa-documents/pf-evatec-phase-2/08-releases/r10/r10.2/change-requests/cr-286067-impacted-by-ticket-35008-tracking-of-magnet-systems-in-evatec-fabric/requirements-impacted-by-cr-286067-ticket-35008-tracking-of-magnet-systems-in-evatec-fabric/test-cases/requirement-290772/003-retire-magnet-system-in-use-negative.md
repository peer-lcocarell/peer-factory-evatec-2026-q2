...
## Expected Outcome
The Retire action is not available for Magnet Systems with status `In Use`. Only Magnet Systems in `In Stock` or `In Inspection` status can be retired. The `In Use` Magnet System remains unchanged.
