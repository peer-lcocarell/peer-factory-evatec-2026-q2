# R10.2 Update for CR 286836 - Edit ERP Hardware Component: Verify validation is displayed when required fields are blank (Unhappy Path)
...
- Tester can restore valid values for all required fields after validation appears.
...
| 5 | Enter valid values in all required fields and click Save. | Validation messages are removed and save completes without errors. |
| 6 | Re-open the same component. | Corrected required-field values are persisted and visible. |

## Expected Outcome
The tester verifies that the system blocks save when required fields are blank, displays clear field-level validation, and permits save only after valid values are restored.
