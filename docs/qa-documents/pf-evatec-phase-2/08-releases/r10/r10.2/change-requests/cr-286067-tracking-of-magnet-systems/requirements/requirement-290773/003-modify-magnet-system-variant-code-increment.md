...
- Tester has permission to modify Magnet Systems.
...
|--------|--------|--------|
| 1 | Verify preconditions. | Preconditions are met. |
...
| 9 | Trigger the `Modify` action again on the same Magnet System. Select to create a new Prototype variant. Enter a different valid unique Article Number and Description. Confirm. | A second Prototype Magnet System Type is created. Variant code `V02` is assigned. The Magnet System is linked to the `V02` Prototype type. A modification history entry is created. |
...
- Duplicate Hardware Component Article Numbers are rejected.
