...
| 2 | Trigger the 'Retire' action. | The Retire confirmation dialog opens. A mandatory reason field is displayed. |
| 3 | Leave the reason field empty and attempt to confirm the retirement. | Retirement is blocked. A required-field validation message is displayed on the reason field. The Magnet System status remains `In Stock`. |
| 4 | Enter a valid reason in the reason field and confirm the retirement. | Validation clears. Retirement completes. The Magnet System status changes to `Retired`. |
| 5 | Open the Magnet System History view. | A retirement history entry is present containing the reason provided, the current user, and the timestamp. |
