# R10.2 Update for CR 286067 - View and Upload Documents for New Magnet System Type: Document Upload and Removal During Creation (Unhappy Path)
...
| 2 | Attempt to upload an unsupported file type. | Upload is rejected, a validation message is shown, and the unsupported file is not listed. |
| 3 | Upload two valid supported document files. | Both valid files upload successfully and appear in the Documents list. |
| 4 | Remove one uploaded document using the remove action. | The selected document is removed immediately and no confirmation dialog is displayed. |
| 5 | Save the new Magnet System Type. | Save completes successfully with the current document list state. |
