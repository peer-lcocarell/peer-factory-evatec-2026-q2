...
|------|--------|-----------------|
| 1 | Open the 'Magnet System Types' page and select a Prototype Magnet System Type. Record baseline values for article number, predecessor, and variant code. | Baseline values are captured before starting redefine as standard. |
| 2 | Trigger the 'Define as Standard' action. | ERP article selection prompt opens. |
| 3 | Choose an ERP article value but select Cancel instead of Confirm. | The dialog closes and no redefine action is committed. |
