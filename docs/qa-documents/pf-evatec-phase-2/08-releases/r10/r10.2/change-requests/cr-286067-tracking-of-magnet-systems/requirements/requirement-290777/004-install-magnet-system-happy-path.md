...
| 3 | Select a valid `Checked Out` Magnet System and confirm the installation. | The selected Magnet System details are shown in the dialog and the installation action completes without error. |
...
| 6 | Navigate to the 'Logbook' for the target tool and apply the Magnet System filter. | A new Logbook entry is present for the install event. The entry includes the Magnet System identifier, serial number, install action, current user, and timestamp. The entry count has increased by one compared to the baseline. |
